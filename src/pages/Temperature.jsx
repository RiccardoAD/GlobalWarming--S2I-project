...
export const Temperature = () => {
  ...
  const impactIcons = [faExclamationTriangle, faWater, faFish, faBug];
  const solutionIcons = [faWind, faSun, faTree, faHandsHelping];
  ...
  return (
    <div className="md:h-[calc(100vh-56px-2rem)] flex flex-col items-center bg-sky-100 dark:bg-[#5b7a92] rounded p-6 ml-2 mr-2 md:mr-4 shadow-md dark:shadow-slate-900 text-sm md:text-base">
      <h1 className="text-4xl md:text-5xl font-bold mb-5 text-blue-900 dark:text-[#c0d6e4]">{t("temperature.title")}</h1>
      <div className="prose prose-lg max-w-screen-lg">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mt-5">
          ...
          <Solutions /> */}
           <Section
            title={t("temperature.impactsTitle")}
            items={t("temperature.impacts", { returnObjects: true })}
            icons={impactIcons}
          />
          <Section
            title={t("temperature.solutionsTitle")}
            items={t("temperature.solutions", { returnObjects: true })}
            icons={solutionIcons}
          />
        </div>
      </div>
